...
test_that("non-prefixed episodes can be created", {
  ...
  suppressMessages({
    second_episode <- create_episode_md("First Markdown", path = tmp, open = FALSE) %>%
...
test_that("un-prefixed episodes can be created", {
  ...
  suppressMessages({
    third_episode <- create_episode_rmd(title, path = tmp, open = FALSE) %>%
...
test_that("draft episodes can be drafted", {
  skip_on_os("windows") # y'all ain't ready for this
  suppressMessages({
    draft_episode_md("ignore-me", path = tmp, open = FALSE)
  })
  expect_equal(get_episodes(tmp), c("introduction.Rmd", "first-markdown.md", "\uC548\uB155-\U0001F62D-kitty.Rmd"))

  suppressMessages({
    draft_episode_rmd("ignore-me-in-r", path = tmp, open = FALSE)
  })
  expect_equal(get_episodes(tmp), c("introduction.Rmd", "first-markdown.md", "\uC548\uB155-\U0001F62D-kitty.Rmd"))
