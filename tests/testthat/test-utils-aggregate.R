...
test_that("aggregate pages do not exorcise self-similar slugs", {
  ...
  suppressMessages({
    create_episode_md("images and pixels", path = res, add = TRUE, open = FALSE)
    create_episode_md("keypoints and others", path = res, add = TRUE, open = FALSE)
    create_episode_md("instructor notes and things", path = res, add = TRUE, open = FALSE)
    create_episode_md("aio stands for an information overload", path = res, add = TRUE, open = FALSE)
  })
