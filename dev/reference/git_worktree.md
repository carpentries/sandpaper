...
#> ℹ Lesson bootstrapped in 3.676007 secs
#> → Bootstrapping example lesson
...
#> ℹ Remote set up in 0.01310587 secs
tok <- Sys.time()
...
#> [md-outputs (root-commit) fd4ce28] Initializing md-outputs branch
#> Running git push sandpaper-local 'HEAD:md-outputs'
#> To /tmp/Rtmps3YM4i/REMOTE-1aec6507a474
...
#> From /tmp/Rtmps3YM4i/REMOTE-1aec6507a474
...
#>   /tmp/Rtmps3YM4i/file1aec4f935544/lesson-example/site/built \
#>   sandpaper-local/md-outputs
#> Preparing worktree (resetting branch 'md-outputs'; was at fd4ce28)
#> branch 'md-outputs' set up to track 'sandpaper-local/md-outputs'.
#> HEAD is now at fd4ce28 Initializing md-outputs branch
#> ::endgroup::
...
#> [gh-pages (root-commit) f5cc01e] Initializing gh-pages branch
...
#>   /tmp/Rtmps3YM4i/file1aec4f935544/lesson-example/site/docs \
#>   sandpaper-local/gh-pages
#> Preparing worktree (resetting branch 'gh-pages'; was at f5cc01e)
#> branch 'gh-pages' set up to track 'sandpaper-local/gh-pages'.
#> HEAD is now at f5cc01e Initializing gh-pages branch
#> ::endgroup::
tok <- Sys.time()
cli::cli_alert_info("Elapsed time: {round(tok - tik, 2)} seconds")
#> ℹ Elapsed time: 0.38 seconds
tik <- Sys.time()
...
tok <- Sys.time()
cli::cli_alert_info("Elapsed time: {round(tok - tik, 2)} seconds")
#> ℹ Elapsed time: 12.82 seconds
tik <- Sys.time()
cli::cli_h1("Clean Up")
#> 
#> ── Clean Up ────────────────────────────────────────────────────────────
cli::cli_alert_info("object db is an expression that evaluates to {.code {db}}")
#> ℹ object db is an expression that evaluates to `sandpaper:::github_worktree_remove("/tmp/Rtmps3YM4i/file1aec4f935544/lesson-example/site/built", "/tmp/Rtmps3YM4i/file1aec4f935544/lesson-example")`
eval(db)
...
#> ℹ object ds is an expression that evaluates to `sandpaper:::github_worktree_remove("/tmp/Rtmps3YM4i/file1aec4f935544/lesson-example/site/docs", "/tmp/Rtmps3YM4i/file1aec4f935544/lesson-example")`
eval(ds)
#> Running git worktree remove --force \
#>   /tmp/Rtmps3YM4i/file1aec4f935544/lesson-example/site/docs
#> $status
...
#> ℹ removing 'sandpaper-local' (/tmp/Rtmps3YM4i/REMOTE-1aec6507a474)
#> /tmp/Rtmps3YM4i/REMOTE-1aec6507a474
