...
#> ℹ Lesson bootstrapped in 3.606802 secs
#> → Bootstrapping example lesson
...
#> ℹ Remote set up in 0.013165 secs
tok <- Sys.time()
...
#> [md-outputs (root-commit) 6e8da9c] Initializing md-outputs branch
#> Running git push sandpaper-local 'HEAD:md-outputs'
#> To /tmp/RtmpACXc8B/REMOTE-1aa412680296
...
#> From /tmp/RtmpACXc8B/REMOTE-1aa412680296
...
#>   /tmp/RtmpACXc8B/file1aa4421f3bc2/lesson-example/site/built \
#>   sandpaper-local/md-outputs
#> Preparing worktree (resetting branch 'md-outputs'; was at 6e8da9c)
#> branch 'md-outputs' set up to track 'sandpaper-local/md-outputs'.
#> HEAD is now at 6e8da9c Initializing md-outputs branch
#> ::endgroup::
...
#> [gh-pages (root-commit) be5eccf] Initializing gh-pages branch
...
#>   /tmp/RtmpACXc8B/file1aa4421f3bc2/lesson-example/site/docs \
#>   sandpaper-local/gh-pages
#> Preparing worktree (resetting branch 'gh-pages'; was at be5eccf)
#> branch 'gh-pages' set up to track 'sandpaper-local/gh-pages'.
#> HEAD is now at be5eccf Initializing gh-pages branch
#> ::endgroup::
tok <- Sys.time()
cli::cli_alert_info("Elapsed time: {round(tok - tik, 2)} seconds")
...
tok <- Sys.time()
cli::cli_alert_info("Elapsed time: {round(tok - tik, 2)} seconds")
#> ℹ Elapsed time: 12.94 seconds
tik <- Sys.time()
cli::cli_h1("Clean Up")
#> 
#> ── Clean Up ────────────────────────────────────────────────────────────
cli::cli_alert_info("object db is an expression that evaluates to {.code {db}}")
#> ℹ object db is an expression that evaluates to `sandpaper:::github_worktree_remove("/tmp/RtmpACXc8B/file1aa4421f3bc2/lesson-example/site/built", "/tmp/RtmpACXc8B/file1aa4421f3bc2/lesson-example")`
eval(db)
...
#> ℹ object ds is an expression that evaluates to `sandpaper:::github_worktree_remove("/tmp/RtmpACXc8B/file1aa4421f3bc2/lesson-example/site/docs", "/tmp/RtmpACXc8B/file1aa4421f3bc2/lesson-example")`
eval(ds)
#> Running git worktree remove --force \
#>   /tmp/RtmpACXc8B/file1aa4421f3bc2/lesson-example/site/docs
#> $status
...
#> ℹ removing 'sandpaper-local' (/tmp/RtmpACXc8B/REMOTE-1aa412680296)
#> /tmp/RtmpACXc8B/REMOTE-1aa412680296
