...
#> ℹ Lesson bootstrapped in 3.129581 secs
#> → Bootstrapping example lesson
...
#> ℹ Remote set up in 0.01344013 secs
tok <- Sys.time()
...
#> [md-outputs (root-commit) cb7d72d] Initializing md-outputs branch
#> Running git push sandpaper-local 'HEAD:md-outputs'
#> To /tmp/RtmpUfNzdK/REMOTE-1c874904a0a7
...
#> From /tmp/RtmpUfNzdK/REMOTE-1c874904a0a7
...
#>   /tmp/RtmpUfNzdK/file1c872adf602b/lesson-example/site/built \
#>   sandpaper-local/md-outputs
#> Preparing worktree (resetting branch 'md-outputs'; was at cb7d72d)
#> branch 'md-outputs' set up to track 'sandpaper-local/md-outputs'.
#> HEAD is now at cb7d72d Initializing md-outputs branch
#> ::endgroup::
...
#> [gh-pages (root-commit) 6b5831d] Initializing gh-pages branch
...
#>   /tmp/RtmpUfNzdK/file1c872adf602b/lesson-example/site/docs \
#>   sandpaper-local/gh-pages
#> Preparing worktree (resetting branch 'gh-pages'; was at 6b5831d)
#> branch 'gh-pages' set up to track 'sandpaper-local/gh-pages'.
#> HEAD is now at 6b5831d Initializing gh-pages branch
#> ::endgroup::
tok <- Sys.time()
cli::cli_alert_info("Elapsed time: {round(tok - tik, 2)} seconds")
...
tok <- Sys.time()
cli::cli_alert_info("Elapsed time: {round(tok - tik, 2)} seconds")
#> ℹ Elapsed time: 12.9 seconds
tik <- Sys.time()
cli::cli_h1("Clean Up")
#> 
#> ── Clean Up ────────────────────────────────────────────────────────────
cli::cli_alert_info("object db is an expression that evaluates to {.code {db}}")
#> ℹ object db is an expression that evaluates to `sandpaper:::github_worktree_remove("/tmp/RtmpUfNzdK/file1c872adf602b/lesson-example/site/built", "/tmp/RtmpUfNzdK/file1c872adf602b/lesson-example")`
eval(db)
...
#> ℹ object ds is an expression that evaluates to `sandpaper:::github_worktree_remove("/tmp/RtmpUfNzdK/file1c872adf602b/lesson-example/site/docs", "/tmp/RtmpUfNzdK/file1c872adf602b/lesson-example")`
eval(ds)
#> Running git worktree remove --force \
#>   /tmp/RtmpUfNzdK/file1c872adf602b/lesson-example/site/docs
#> $status
...
#> ℹ removing 'sandpaper-local' (/tmp/RtmpUfNzdK/REMOTE-1c874904a0a7)
#> /tmp/RtmpUfNzdK/REMOTE-1c874904a0a7
