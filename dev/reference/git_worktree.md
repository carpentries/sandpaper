...
#> ℹ Lesson bootstrapped in 3.207011 secs
#> → Bootstrapping example lesson
...
#> ℹ Remote set up in 0.01378465 secs
tok <- Sys.time()
...
#> [md-outputs (root-commit) 828ec77] Initializing md-outputs branch
#> Running git push sandpaper-local 'HEAD:md-outputs'
#> To /tmp/Rtmp9MemTm/REMOTE-199e312f74ec
...
#> From /tmp/Rtmp9MemTm/REMOTE-199e312f74ec
...
#>   /tmp/Rtmp9MemTm/file199e1e4dea9e/lesson-example/site/built \
#>   sandpaper-local/md-outputs
#> Preparing worktree (resetting branch 'md-outputs'; was at 828ec77)
#> branch 'md-outputs' set up to track 'sandpaper-local/md-outputs'.
#> HEAD is now at 828ec77 Initializing md-outputs branch
#> ::endgroup::
...
#> [gh-pages (root-commit) 4e1ff52] Initializing gh-pages branch
...
#>   /tmp/Rtmp9MemTm/file199e1e4dea9e/lesson-example/site/docs \
#>   sandpaper-local/gh-pages
#> Preparing worktree (resetting branch 'gh-pages'; was at 4e1ff52)
#> branch 'gh-pages' set up to track 'sandpaper-local/gh-pages'.
#> HEAD is now at 4e1ff52 Initializing gh-pages branch
#> ::endgroup::
tok <- Sys.time()
cli::cli_alert_info("Elapsed time: {round(tok - tik, 2)} seconds")
...
tok <- Sys.time()
cli::cli_alert_info("Elapsed time: {round(tok - tik, 2)} seconds")
#> ℹ Elapsed time: 13.09 seconds
tik <- Sys.time()
cli::cli_h1("Clean Up")
#> 
#> ── Clean Up ────────────────────────────────────────────────────────────
cli::cli_alert_info("object db is an expression that evaluates to {.code {db}}")
#> ℹ object db is an expression that evaluates to `sandpaper:::github_worktree_remove("/tmp/Rtmp9MemTm/file199e1e4dea9e/lesson-example/site/built", "/tmp/Rtmp9MemTm/file199e1e4dea9e/lesson-example")`
eval(db)
...
#> ℹ object ds is an expression that evaluates to `sandpaper:::github_worktree_remove("/tmp/Rtmp9MemTm/file199e1e4dea9e/lesson-example/site/docs", "/tmp/Rtmp9MemTm/file199e1e4dea9e/lesson-example")`
eval(ds)
#> Running git worktree remove --force \
#>   /tmp/Rtmp9MemTm/file199e1e4dea9e/lesson-example/site/docs
#> $status
...
#> ℹ removing 'sandpaper-local' (/tmp/Rtmp9MemTm/REMOTE-199e312f74ec)
#> /tmp/Rtmp9MemTm/REMOTE-199e312f74ec
