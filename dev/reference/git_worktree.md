...
#> ℹ Lesson bootstrapped in 3.045104 secs
#> → Bootstrapping example lesson
...
#> ℹ Remote set up in 0.01390624 secs
tok <- Sys.time()
...
#> [md-outputs (root-commit) 479a433] Initializing md-outputs branch
#> Running git push sandpaper-local 'HEAD:md-outputs'
#> To /tmp/Rtmp2oQNAm/REMOTE-1c253c9897b4
...
#> From /tmp/Rtmp2oQNAm/REMOTE-1c253c9897b4
...
#>   /tmp/Rtmp2oQNAm/file1c2550bfae35/lesson-example/site/built \
#>   sandpaper-local/md-outputs
#> Preparing worktree (resetting branch 'md-outputs'; was at 479a433)
#> branch 'md-outputs' set up to track 'sandpaper-local/md-outputs'.
#> HEAD is now at 479a433 Initializing md-outputs branch
#> ::endgroup::
...
#> [gh-pages (root-commit) c2fc992] Initializing gh-pages branch
...
#>   /tmp/Rtmp2oQNAm/file1c2550bfae35/lesson-example/site/docs \
#>   sandpaper-local/gh-pages
#> Preparing worktree (resetting branch 'gh-pages'; was at c2fc992)
#> branch 'gh-pages' set up to track 'sandpaper-local/gh-pages'.
#> HEAD is now at c2fc992 Initializing gh-pages branch
#> ::endgroup::
tok <- Sys.time()
cli::cli_alert_info("Elapsed time: {round(tok - tik, 2)} seconds")
...
tok <- Sys.time()
cli::cli_alert_info("Elapsed time: {round(tok - tik, 2)} seconds")
#> ℹ Elapsed time: 12.8 seconds
tik <- Sys.time()
cli::cli_h1("Clean Up")
#> 
#> ── Clean Up ────────────────────────────────────────────────────────────
cli::cli_alert_info("object db is an expression that evaluates to {.code {db}}")
#> ℹ object db is an expression that evaluates to `sandpaper:::github_worktree_remove("/tmp/Rtmp2oQNAm/file1c2550bfae35/lesson-example/site/built", "/tmp/Rtmp2oQNAm/file1c2550bfae35/lesson-example")`
eval(db)
...
#> ℹ object ds is an expression that evaluates to `sandpaper:::github_worktree_remove("/tmp/Rtmp2oQNAm/file1c2550bfae35/lesson-example/site/docs", "/tmp/Rtmp2oQNAm/file1c2550bfae35/lesson-example")`
eval(ds)
#> Running git worktree remove --force \
#>   /tmp/Rtmp2oQNAm/file1c2550bfae35/lesson-example/site/docs
#> $status
...
#> ℹ removing 'sandpaper-local' (/tmp/Rtmp2oQNAm/REMOTE-1c253c9897b4)
#> /tmp/Rtmp2oQNAm/REMOTE-1c253c9897b4
