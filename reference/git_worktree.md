...
#> ℹ Lesson bootstrapped in 3.225727 secs
#> → Bootstrapping example lesson
...
#> ℹ Remote set up in 0.01364923 secs
tok <- Sys.time()
...
#> [md-outputs (root-commit) e573378] Initializing md-outputs branch
#> Running git push sandpaper-local 'HEAD:md-outputs'
#> To /tmp/Rtmph9vhkW/REMOTE-1bff599ee4e6
...
#> From /tmp/Rtmph9vhkW/REMOTE-1bff599ee4e6
...
#>   /tmp/Rtmph9vhkW/file1bff4957d9a2/lesson-example/site/built \
#>   sandpaper-local/md-outputs
#> Preparing worktree (resetting branch 'md-outputs'; was at e573378)
#> branch 'md-outputs' set up to track 'sandpaper-local/md-outputs'.
#> HEAD is now at e573378 Initializing md-outputs branch
#> ::endgroup::
...
#> [gh-pages (root-commit) c467eba] Initializing gh-pages branch
...
#>   /tmp/Rtmph9vhkW/file1bff4957d9a2/lesson-example/site/docs \
#>   sandpaper-local/gh-pages
#> Preparing worktree (resetting branch 'gh-pages'; was at c467eba)
#> branch 'gh-pages' set up to track 'sandpaper-local/gh-pages'.
#> HEAD is now at c467eba Initializing gh-pages branch
#> ::endgroup::
tok <- Sys.time()
cli::cli_alert_info("Elapsed time: {round(tok - tik, 2)} seconds")
...
tok <- Sys.time()
cli::cli_alert_info("Elapsed time: {round(tok - tik, 2)} seconds")
#> ℹ Elapsed time: 13.8 seconds
tik <- Sys.time()
cli::cli_h1("Clean Up")
#> 
#> ── Clean Up ────────────────────────────────────────────────────────────
cli::cli_alert_info("object db is an expression that evaluates to {.code {db}}")
#> ℹ object db is an expression that evaluates to `sandpaper:::github_worktree_remove("/tmp/Rtmph9vhkW/file1bff4957d9a2/lesson-example/site/built", "/tmp/Rtmph9vhkW/file1bff4957d9a2/lesson-example")`
eval(db)
...
#> ℹ object ds is an expression that evaluates to `sandpaper:::github_worktree_remove("/tmp/Rtmph9vhkW/file1bff4957d9a2/lesson-example/site/docs", "/tmp/Rtmph9vhkW/file1bff4957d9a2/lesson-example")`
eval(ds)
#> Running git worktree remove --force \
#>   /tmp/Rtmph9vhkW/file1bff4957d9a2/lesson-example/site/docs
#> $status
...
#> ℹ removing 'sandpaper-local' (/tmp/Rtmph9vhkW/REMOTE-1bff599ee4e6)
#> /tmp/Rtmph9vhkW/REMOTE-1bff599ee4e6
