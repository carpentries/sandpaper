...
#> ℹ Lesson bootstrapped in 3.584882 secs
#> → Bootstrapping example lesson
...
#> ℹ Remote set up in 0.009504557 secs
tok <- Sys.time()
...
#> [md-outputs (root-commit) f0e7588] Initializing md-outputs branch
#> Running git push sandpaper-local 'HEAD:md-outputs'
#> To /tmp/RtmpYhnOfg/REMOTE-1a1d161d4e24
...
#> From /tmp/RtmpYhnOfg/REMOTE-1a1d161d4e24
...
#>   /tmp/RtmpYhnOfg/file1a1d7d645c53/lesson-example/site/built \
#>   sandpaper-local/md-outputs
#> Preparing worktree (resetting branch 'md-outputs'; was at f0e7588)
#> branch 'md-outputs' set up to track 'sandpaper-local/md-outputs'.
#> HEAD is now at f0e7588 Initializing md-outputs branch
#> ::endgroup::
...
#> [gh-pages (root-commit) dc4e9c1] Initializing gh-pages branch
...
#>   /tmp/RtmpYhnOfg/file1a1d7d645c53/lesson-example/site/docs \
#>   sandpaper-local/gh-pages
#> Preparing worktree (resetting branch 'gh-pages'; was at dc4e9c1)
#> branch 'gh-pages' set up to track 'sandpaper-local/gh-pages'.
#> HEAD is now at dc4e9c1 Initializing gh-pages branch
#> ::endgroup::
tok <- Sys.time()
cli::cli_alert_info("Elapsed time: {round(tok - tik, 2)} seconds")
...
tok <- Sys.time()
cli::cli_alert_info("Elapsed time: {round(tok - tik, 2)} seconds")
#> ℹ Elapsed time: 12.52 seconds
tik <- Sys.time()
cli::cli_h1("Clean Up")
#> 
#> ── Clean Up ────────────────────────────────────────────────────────────
cli::cli_alert_info("object db is an expression that evaluates to {.code {db}}")
#> ℹ object db is an expression that evaluates to `sandpaper:::github_worktree_remove("/tmp/RtmpYhnOfg/file1a1d7d645c53/lesson-example/site/built", "/tmp/RtmpYhnOfg/file1a1d7d645c53/lesson-example")`
eval(db)
...
#> ℹ object ds is an expression that evaluates to `sandpaper:::github_worktree_remove("/tmp/RtmpYhnOfg/file1a1d7d645c53/lesson-example/site/docs", "/tmp/RtmpYhnOfg/file1a1d7d645c53/lesson-example")`
eval(ds)
#> Running git worktree remove --force \
#>   /tmp/RtmpYhnOfg/file1a1d7d645c53/lesson-example/site/docs
#> $status
...
#> ℹ removing 'sandpaper-local' (/tmp/RtmpYhnOfg/REMOTE-1a1d161d4e24)
#> /tmp/RtmpYhnOfg/REMOTE-1a1d161d4e24
