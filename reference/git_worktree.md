...
#> ℹ Lesson bootstrapped in 3.60778 secs
#> → Bootstrapping example lesson
...
#> ℹ Remote set up in 0.01287055 secs
tok <- Sys.time()
cli::cli_alert_info("Elapsed time: {round(tok - tik, 2)} seconds")
#> ℹ Elapsed time: 3.64 seconds
tik <- Sys.time()
...
#> [md-outputs (root-commit) 859ad65] Initializing md-outputs branch
#> Running git push sandpaper-local 'HEAD:md-outputs'
#> To /tmp/RtmpQHhDUx/REMOTE-1ba29537569
...
#> From /tmp/RtmpQHhDUx/REMOTE-1ba29537569
...
#>   /tmp/RtmpQHhDUx/file1ba24ccaf90c/lesson-example/site/built \
#>   sandpaper-local/md-outputs
#> Preparing worktree (resetting branch 'md-outputs'; was at 859ad65)
#> branch 'md-outputs' set up to track 'sandpaper-local/md-outputs'.
#> HEAD is now at 859ad65 Initializing md-outputs branch
#> ::endgroup::
...
#> [gh-pages (root-commit) 8e4d194] Initializing gh-pages branch
...
#>   /tmp/RtmpQHhDUx/file1ba24ccaf90c/lesson-example/site/docs \
#>   sandpaper-local/gh-pages
#> Preparing worktree (resetting branch 'gh-pages'; was at 8e4d194)
#> branch 'gh-pages' set up to track 'sandpaper-local/gh-pages'.
#> HEAD is now at 8e4d194 Initializing gh-pages branch
#> ::endgroup::
tok <- Sys.time()
cli::cli_alert_info("Elapsed time: {round(tok - tik, 2)} seconds")
#> ℹ Elapsed time: 0.44 seconds
tik <- Sys.time()
...
tok <- Sys.time()
cli::cli_alert_info("Elapsed time: {round(tok - tik, 2)} seconds")
#> ℹ Elapsed time: 13.46 seconds
tik <- Sys.time()
cli::cli_h1("Clean Up")
#> 
#> ── Clean Up ────────────────────────────────────────────────────────────
cli::cli_alert_info("object db is an expression that evaluates to {.code {db}}")
#> ℹ object db is an expression that evaluates to `sandpaper:::github_worktree_remove("/tmp/RtmpQHhDUx/file1ba24ccaf90c/lesson-example/site/built", "/tmp/RtmpQHhDUx/file1ba24ccaf90c/lesson-example")`
eval(db)
...
#> ℹ object ds is an expression that evaluates to `sandpaper:::github_worktree_remove("/tmp/RtmpQHhDUx/file1ba24ccaf90c/lesson-example/site/docs", "/tmp/RtmpQHhDUx/file1ba24ccaf90c/lesson-example")`
eval(ds)
#> Running git worktree remove --force \
#>   /tmp/RtmpQHhDUx/file1ba24ccaf90c/lesson-example/site/docs
#> $status
...
#> ℹ removing 'sandpaper-local' (/tmp/RtmpQHhDUx/REMOTE-1ba29537569)
#> /tmp/RtmpQHhDUx/REMOTE-1ba29537569
