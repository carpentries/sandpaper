...
#> ℹ Lesson bootstrapped in 3.146263 secs
#> → Bootstrapping example lesson
...
#> ℹ Remote set up in 0.01303315 secs
tok <- Sys.time()
...
#> [md-outputs (root-commit) 730ea4a] Initializing md-outputs branch
#> Running git push sandpaper-local 'HEAD:md-outputs'
#> To /tmp/Rtmp4syuyP/REMOTE-1cc726ec4475
...
#> From /tmp/Rtmp4syuyP/REMOTE-1cc726ec4475
...
#>   /tmp/Rtmp4syuyP/file1cc763cd3e53/lesson-example/site/built \
#>   sandpaper-local/md-outputs
#> Preparing worktree (resetting branch 'md-outputs'; was at 730ea4a)
#> branch 'md-outputs' set up to track 'sandpaper-local/md-outputs'.
#> HEAD is now at 730ea4a Initializing md-outputs branch
#> ::endgroup::
...
#> [gh-pages (root-commit) 7010c7f] Initializing gh-pages branch
...
#>   /tmp/Rtmp4syuyP/file1cc763cd3e53/lesson-example/site/docs \
#>   sandpaper-local/gh-pages
#> Preparing worktree (resetting branch 'gh-pages'; was at 7010c7f)
#> branch 'gh-pages' set up to track 'sandpaper-local/gh-pages'.
#> HEAD is now at 7010c7f Initializing gh-pages branch
#> ::endgroup::
tok <- Sys.time()
cli::cli_alert_info("Elapsed time: {round(tok - tik, 2)} seconds")
...
#> ℹ object db is an expression that evaluates to `sandpaper:::github_worktree_remove("/tmp/Rtmp4syuyP/file1cc763cd3e53/lesson-example/site/built", "/tmp/Rtmp4syuyP/file1cc763cd3e53/lesson-example")`
eval(db)
...
#> ℹ object ds is an expression that evaluates to `sandpaper:::github_worktree_remove("/tmp/Rtmp4syuyP/file1cc763cd3e53/lesson-example/site/docs", "/tmp/Rtmp4syuyP/file1cc763cd3e53/lesson-example")`
eval(ds)
#> Running git worktree remove --force \
#>   /tmp/Rtmp4syuyP/file1cc763cd3e53/lesson-example/site/docs
#> $status
...
#> ℹ removing 'sandpaper-local' (/tmp/Rtmp4syuyP/REMOTE-1cc726ec4475)
#> /tmp/Rtmp4syuyP/REMOTE-1cc726ec4475
