...
#> ℹ Lesson bootstrapped in 3.141161 secs
#> → Bootstrapping example lesson
...
#> ℹ Remote set up in 0.01301217 secs
tok <- Sys.time()
...
#> [md-outputs (root-commit) 52620b3] Initializing md-outputs branch
#> Running git push sandpaper-local 'HEAD:md-outputs'
#> To /tmp/Rtmpfmukg6/REMOTE-1bf32154d389
...
#> From /tmp/Rtmpfmukg6/REMOTE-1bf32154d389
...
#>   /tmp/Rtmpfmukg6/file1bf353cfe809/lesson-example/site/built \
#>   sandpaper-local/md-outputs
#> Preparing worktree (resetting branch 'md-outputs'; was at 52620b3)
#> branch 'md-outputs' set up to track 'sandpaper-local/md-outputs'.
#> HEAD is now at 52620b3 Initializing md-outputs branch
#> ::endgroup::
...
#> [gh-pages (root-commit) 449c613] Initializing gh-pages branch
...
#>   /tmp/Rtmpfmukg6/file1bf353cfe809/lesson-example/site/docs \
#>   sandpaper-local/gh-pages
#> Preparing worktree (resetting branch 'gh-pages'; was at 449c613)
#> branch 'gh-pages' set up to track 'sandpaper-local/gh-pages'.
#> HEAD is now at 449c613 Initializing gh-pages branch
#> ::endgroup::
tok <- Sys.time()
cli::cli_alert_info("Elapsed time: {round(tok - tik, 2)} seconds")
...
tok <- Sys.time()
cli::cli_alert_info("Elapsed time: {round(tok - tik, 2)} seconds")
#> ℹ Elapsed time: 12.85 seconds
tik <- Sys.time()
cli::cli_h1("Clean Up")
#> 
#> ── Clean Up ────────────────────────────────────────────────────────────
cli::cli_alert_info("object db is an expression that evaluates to {.code {db}}")
#> ℹ object db is an expression that evaluates to `sandpaper:::github_worktree_remove("/tmp/Rtmpfmukg6/file1bf353cfe809/lesson-example/site/built", "/tmp/Rtmpfmukg6/file1bf353cfe809/lesson-example")`
eval(db)
...
#> ℹ object ds is an expression that evaluates to `sandpaper:::github_worktree_remove("/tmp/Rtmpfmukg6/file1bf353cfe809/lesson-example/site/docs", "/tmp/Rtmpfmukg6/file1bf353cfe809/lesson-example")`
eval(ds)
#> Running git worktree remove --force \
#>   /tmp/Rtmpfmukg6/file1bf353cfe809/lesson-example/site/docs
#> $status
...
#> ℹ removing 'sandpaper-local' (/tmp/Rtmpfmukg6/REMOTE-1bf32154d389)
#> /tmp/Rtmpfmukg6/REMOTE-1bf32154d389
