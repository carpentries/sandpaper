...
To pin a specific package version, you can use the `pin_version()` function. For
example, here's how you would pin the {cowsay} package to version 0.7.0:
...
```{r fetch-updates, eval = FALSE}
fetch_updates(prompt = TRUE)
```

```{r fetch-updates-show, echo = FALSE}
