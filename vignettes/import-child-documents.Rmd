...
Let's say you wanted to refererence a figure `episodes/fig/one.png` from
the child document, but instead it's located at
...
ep$write(fs::path(lsn, "episodes"), format = "Rmd")
lsn_obj <- snd$this_lesson(lsn)
lsn_obj$children[[1]]$add_md("![example](fig/one.png){alt='example figure'}")
...
and `episodes/files/children/child.Rmd` would look like this, where the figure
is referenced _in the context of the build parent directory_):
...
is all in the context of the parent document in the `site/built` directory:
...
lsn_obj$load_built()
lsn_obj$built[["site/built/introduction.md"]]$tail(47)
```
