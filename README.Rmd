...
| [{testthat}] version 3 | runs tests for the package. If you aren't developing, you shouldn't worry |
| [{pkgdown}] | provides utilities to generate templates | 
...
**Website Creation and Validation**
 ...
 - `build_markdown_md()` renders an individual file to markdown (internal use)
 - `build_markdown_html()` renders a built markdown file to html (internal use)
...
the `set_schedule()` command to build the schedule in your `config.yaml` file:
...
[{dovetail}]: https://github.com/carpentries/dovetail#readme
